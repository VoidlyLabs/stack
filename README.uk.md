...
Volumes зберігаються між rebuilds і restarts.

## Дані Та Volumes

| Volume | Вміст |
| --- | --- |
| `voidly_mongo_data` | Файли бази даних MongoDB. |
| `voidly_core_uploads` | Uploaded files, які API віддає на `/uploads`. |

`sh deploy.sh down` видаляє containers, але залишає volumes. Для видалення даних використовуйте Docker volume commands явно і лише після backup.
